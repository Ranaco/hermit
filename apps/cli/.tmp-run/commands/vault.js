...
vaultCommand
    ...
    ui.cards(vaults.map((vault) => ({
        id: vault.id,
        name: vault.name,
        active: vault.id === authStore.getVaultId(),
        fields: [
            ...(vault.description ? [{ value: vault.description, overflow: "wrap" }] : []),
...
vaultCommand
    ...
    const name = opts.name ||
        (await promptInput({ message: "Vault name:", validate: (value) => (value.trim() ? true : "Name is required") }, "Vault name is required in non-interactive mode."));
    ...
    if (opts.password) {
        ...
        const confirmation = await promptPassword({ message: "Confirm vault password:" }, "Vault password confirmation is required in non-interactive mode.");
