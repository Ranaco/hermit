...
async function renderGroupTree(vaultId, parentId, prefix = "") {
...
groupCommand
    ...
    ui.cards(groups.map((group) => ({
        id: group.id,
        name: group.name,
        fields: [
            ...(group.description ? [{ value: group.description, overflow: "wrap" }] : []),
...
groupCommand
    ...
    const name = opts.name ||
        (await promptInput({ message: "Group name:", validate: (value) => (value.trim() ? true : "Name is required") }, "Group name is required in non-interactive mode."));
...
groupCommand
    ...
    .action((query, opts) => runCommand(async () => {
    requireAuth();
    const vault = opts.vault ? await resolveVault(opts.vault) : await requireActiveVault();
...
groupCommand
    ...
    if (!opts.yes) {
        const confirmed = await promptConfirm({ message: `Delete group "${group.name}"?`, default: false }, "Use --yes when deleting a secret group in non-interactive mode.");
        if (!confirmed) {
            ui.warn("Cancelled");
            ui.newline();
            return;
