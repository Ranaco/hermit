...
authCommand
    ...
    const password = await promptPassword({ message: "Password:" }, "Password is required in non-interactive mode.");
    let result;
    try {
...
mfaCommand
    ...
    .action(() => runCommand(async () => {
    requireAuth();
    const password = await promptPassword({ message: "Current password:" }, "Current password is required in non-interactive mode.");
    const mfaToken = await promptInput({
        message: "MFA token:",
        validate: (value) => (/^\d{6}$/.test(value) ? true : "Enter a 6-digit token"),
    }, "MFA token is required in non-interactive mode.");
...
authCommand.addCommand(new Command("setup-mfa").description("Compatibility alias for `auth mfa setup`").action(() => runCommand(async () => {
    requireAuth();
    const result = await sdk.setupMfa();
    renderData({ success: true, ...result });
    ui.panel("MFA Setup", [
