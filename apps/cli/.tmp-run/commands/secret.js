...
const valueTypes = ["STRING", "JSON", "NUMBER", "BOOLEAN", "MULTILINE"];
async function resolveKeyId(vaultId, requestedKeyId) {
    if (requestedKeyId)
...
secretCommand
    ...
    .action((nameArg, valueArg, opts) => runCommand(async () => {
    requireAuth();
    const vault = opts.vault ? await resolveVault(opts.vault) : await requireActiveVault();
    const group = opts.path
        ? await resolveGroupByPath(vault.id, opts.path)
    ...
    const name = nameArg ||
        (await promptInput({
            message: "Secret name:",
            validate: (value) => /^[a-zA-Z0-9-_. ]+$/.test(value) ? true : "Invalid secret name",
    ...
    if (opts.password) {
        ...
        const confirmation = await promptPassword({ message: "Confirm secret password:" }, "Secret password confirmation is required in non-interactive mode.");
...
secretCommand
    ...
    .action((nameArg, opts) => runCommand(async () => {
    requireAuth();
    const vault = opts.vault ? await resolveVault(opts.vault) : await requireActiveVault();
    const group = opts.path
        ? await resolveGroupByPath(vault.id, opts.path)
    ...
    try {
        ...
        if (apiError.statusCode === 403 && code === "SECRET_PASSWORD_REQUIRED") {
            secretPassword = await promptPassword({ message: "Secret password:" }, "Secret password is required in non-interactive mode.");
            ...
            vaultPassword = await promptPassword({ message: "Vault password:" }, "Vault password is required in non-interactive mode.");
...
secretCommand
    ...
    if (!opts.yes) {
        const confirmed = await promptConfirm({ message: `Delete secret "${secret.name}"?`, default: false }, "Use --yes when deleting a secret in non-interactive mode.");
        if (!confirmed) {
            ui.warn("Cancelled");
            ui.newline();
            return;
