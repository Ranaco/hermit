...
keyCommand
    ...
    const name = opts.name ||
        (await promptInput({ message: "Key name:", validate: (value) => (value.trim() ? true : "Name is required") }, "Key name is required in non-interactive mode."));
...
keyCommand
    ...
    .action((id, opts) => runCommand(async () => {
    requireAuth();
    const key = await sdk.getKey(id);
    if (!opts.yes) {
        const confirmed = await promptConfirm({ message: `Delete key "${key.name}"?`, default: false }, "Use --yes when deleting a key in non-interactive mode.");
        if (!confirmed) {
            ui.warn("Cancelled");
            ui.newline();
            return;
